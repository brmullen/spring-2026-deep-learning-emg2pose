...
# Modified: standalone module with relative imports
...
from .utils import get_contiguous_ones, get_ik_failures_mask

# Default path to per-user scalers
DEFAULT_SCALER_PATH = Path(__file__).resolve().parent.parent / "scaler" / "user_scalers.pkl"


def load_user_scalers(path: Path = DEFAULT_SCALER_PATH) -> dict:
	"""Load per-user StandardScaler objects from a pickle file.

	Args:
		path: Path to the pickle file containing a dict mapping
		      user IDs to sklearn StandardScaler instances.

	Returns:
		Dict mapping user ID strings to StandardScaler objects.
	"""
	with open(path, "rb") as f:
		return pickle.load(f)


def zscore_emg(emg: np.ndarray, scaler) -> np.ndarray:
	"""Apply z-score normalization to EMG data using a user's scaler.

	Args:
		emg: EMG array of shape (T, 16).
		scaler: A fitted sklearn StandardScaler with mean_ and scale_
		        of shape (16,).

	Returns:
		Z-scored EMG array of shape (T, 16).
	"""
	return (emg - scaler.mean_) / scaler.scale_


@dataclass
class Emg2PoseSessionData:
	"""A read-only interface to a single emg2pose session file stored in HDF5 format.
	...
	EMG data is accessed directly from disk. When wrapping this interface
	...
	def slice(self, start_t: float = -np.inf, end_t: float = np.inf) -> np.ndarray:
		...
		Args:
			start_t: Start time of the window in absolute unix time.
				Defaults to selecting from the beginning of the session.
			end_t: End time of the window in absolute unix time.
	...
	def timestamps(self) -> np.ndarray:
		...
		NOTE: This reads the entire sequence of timestamps from the underlying
	...
	def no_ik_failure(self):
		"""Boolean mask where True indicates no inverse kinematics failure."""
...
class WindowedEmgDataset(torch.utils.data.Dataset):
	"""A PyTorch Dataset that iterates over EMG windows of configurable
	length and stride from an Emg2PoseSessionData HDF5 file.

	Args:
		hdf5_path: Path to the session file in HDF5 format.
		window_length: Size of each window in samples. None for entire session.
		stride: Stride between consecutive windows. None sets to window_length.
		padding: (left, right) contextual padding in raw EMG samples.
		jitter: If True, randomly jitter window offsets for training variability.
		transform: Callable that converts a numpy structured array window to a Tensor.
		skip_ik_failures: If True, only produce windows with no IK failures.
	"""
	...
	@property
	def session(self):
		"""Lazy-loaded Emg2PoseSessionData instance."""
	...
	def blocks(self) -> list[tuple[int, int]]:
		"""List of (start, end) sample indices to include in the dataset."""
	...
	def precompute_windows(self) -> list[tuple[int, int]]:
		"""For each dataset index, precompute the EMG start and end indices."""
		windows = []
	...
	def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
		...
		# Apply per-user z-score normalization to EMG if a scaler is set
		if self.scaler is not None:
			window = window.copy()
			window[Emg2PoseSessionData.EMG] = zscore_emg(
				window[Emg2PoseSessionData.EMG], self.scaler
			)
		...
		return {
			"emg": emg.T,  # (channels, time)
			"joint_angles": joint_angles.T,  # (channels, time)
			"no_ik_failure": no_ik_failure,  # (time,)
